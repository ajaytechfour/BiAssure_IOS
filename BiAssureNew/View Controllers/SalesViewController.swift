...
class SalesViewController: UIViewController,UIPickerViewDelegate,UIPickerViewDataSource,UITableViewDataSource,UITableViewDelegate,ChartViewDelegate,SSMaterialCalendarPickerDelegate {
...
//        }
    ...
    func CurentDateWebserviceCallingMethod()
    {
        ...
        manager .post("\(Base_Url)SalesOverview", parameters: dictRegion, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
    ...
    func PreviousDateWebserviceCallingMethod()
    {
     ...
     //   SVProgressHUD.setDefaultMaskType(.black)
        let Base_Url = "http://bi.servassure.net/api/"
        manager .post("\(Base_Url)SalesOverview", parameters: dictPreRegion, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
            if let jsonResponse = responseObject as? [String: AnyObject]
            {
                print("JSON: \(jsonResponse)")
                let info : NSDictionary = jsonResponse as NSDictionary
...
//                        self.preenddate = addtionInfo["end_date"] as! String
    ...
    func ChartDataShowMethod()
    {
        ...
        // SVProgressHUD.setDefaultMaskType(.black)
        let Base_Url = "http://bi.servassure.net/api/"
        manager .post("\(Base_Url)SalesOverviewOEMLevel2", parameters: dictStartDateEndDate, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
            if let jsonResponse = responseObject as? [String: AnyObject]
            {
                print("JSON: \(jsonResponse)")
                let info : NSDictionary = jsonResponse as NSDictionary
