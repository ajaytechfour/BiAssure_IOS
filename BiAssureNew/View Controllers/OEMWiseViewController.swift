...
class OEMWiseViewController: UIViewController,UIPickerViewDelegate,UIPickerViewDataSource,SSMaterialCalendarPickerDelegate,UITableViewDelegate,UITableViewDataSource,ChartViewDelegate {
    ...
    func CurentDateWebserviceCallingMethod()
    {
       ...
       // SVProgressHUD.setDefaultMaskType(.black)
        let Base_Url = "http://bi.servassure.net/api/SalesOverview"
        
        manager .post(NSString.init(format: "\(Base_Url)" as NSString, 0) as String, parameters: dictRegion, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
    ...
    func PreviousDateWebserviceCallingMethod()
    {
        ...
        //SVProgressHUD.setDefaultMaskType(.black)
        let Base_Url = "http://bi.servassure.net/api/SalesOverview"
        manager .post(NSString.init(format: "\(Base_Url)" as NSString, 0) as String, parameters: dictPreRegion, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
            if let jsonResponse = responseObject as? [String: AnyObject]
            {
                print("JSON: \(jsonResponse)")
                let info : NSDictionary = jsonResponse as NSDictionary
    ...
    func ChartDataShowMethod()
    {
        ...
        // SVProgressHUD.show()
        // SVProgressHUD.setDefaultMaskType(.black)
        let Base_Url = "http://bi.servassure.net/api/SalesOverviewOEMLevel2"
        manager .post(NSString.init(format: "\(Base_Url)" as NSString, 0) as String, parameters: dictStartDateEndDate, headers: ["charset=UTF-8" : "application/x-www-form-urlencoded"], progress: nil, success: { (task: URLSessionDataTask!, responseObject: Any!) in
            if let jsonResponse = responseObject as? [String: AnyObject]
            {
                print("JSON: \(jsonResponse)")
                let info : NSDictionary = jsonResponse as NSDictionary
